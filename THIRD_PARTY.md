...
CamillaApp downloads CamillaDSP from its official upstream release during setup. The app bundle includes the source-built System Audio Bridge HAL driver so users do not need a separate loopback download.
...
CamillaApp is therefore distributed under GPL-3.0-only. Each component remains subject to its upstream copyright and license terms. Recheck those terms before changing how dependencies are acquired or bundled.
