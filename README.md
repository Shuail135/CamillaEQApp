<p align="center">
  <img src="Sources/CamillaApp/icon.png" width="128" height="128" alt="CamillaApp duck icon">
</p>

<h1 align="center">CamillaApp</h1>
...
CamillaEQApp now renamed as CamillaApp. CamillaApp gives macOS a proper **system-wide parametric equalizer** with an easy-to-use app interface. Each EQ band gives you direct control over its **frequency**, **gain**, **Q (bandwidth)**, filter type, and on/off state, so you can make precise corrections instead of relying on a basic bass-and-treble control.

Your EQ applies to audio from the whole Mac. Choose your headphones, speakers, DAC, or audio interface; shape the sound visually or import an Equalizer APO preset; then turn on **System-wide EQ**. CamillaApp can remember separate settings for each output device and apply the correct profile automatically when you switch devices.
...
![CamillaApp parametric equalizer interface](screenshot/equalizer.png)

### Device profile

Keep separate EQ and routing settings for each pair of headphones, speakers, DAC, or audio interface.

![CamillaApp output-device profile example](screenshot/profile_example.png)

### Guided setup

Install and verify CamillaDSP and the bundled System Audio Bridge driver from inside the app.

![CamillaApp setup interface](screenshot/setup_interface.png)
...
- **Start at login:** optionally launch CamillaApp when you sign in to your Mac.
- **Set up required components:** install CamillaDSP and the bundled audio driver from the app's Setup page.

CamillaApp uses one active sound profile at a time. You can save as many profiles as you want, but your Mac's audio only passes through one EQ at once.

## How it works

CamillaApp safely sends your Mac's audio through the equalizer before it reaches your headphones or speakers:
...
When you turn EQ off, CamillaApp stops processing and returns audio to the physical output.
...
Open the repository's **Releases** page and download `CamillaApp-v0.x.x-app.zip`.

### 2. Move it to Applications

Double-click the downloaded ZIP if your Mac did not extract it automatically. Then drag `CamillaApp.app` into your Mac's **Applications** folder.
...
1. Try to open `CamillaApp` from your Applications folder, then select **Done** when macOS blocks it.
2. Open **System Settings → Privacy & Security**.
3. Scroll down to **Security** and select **Open Anyway** beside CamillaApp.
...
1. In CamillaApp, choose **Setup** in the sidebar.
...
5. Reopen CamillaApp after restarting.
...
| --- | --- |
| `CamillaApp-v0.x.x-app.zip` | Most users—extract it and move the app to Applications. |
| `CamillaApp-v0.x.x-build-folder.zip` | Developers who want the complete generated build folder. |
...
- **System Audio Bridge is missing after installation:** restart your Mac, then reopen CamillaApp.
...
System Audio Bridge is CamillaApp's output-only Core Audio driver. When an EQ
profile is activated, macOS sends system audio to a virtual output named after
that profile. The driver passes the audio to CamillaApp, CamillaDSP applies
...
one tells CamillaApp which profile to activate. The temporary selector is then
...
`Device:` is recognized but ignored because device selection belongs to CamillaApp profiles. `Channel:` produces a warning because the current release applies filters to both stereo channels.
...
```bash
git clone https://github.com/Shuail135/CamillaApp.git
cd CamillaApp
./build-app.sh
open dist/CamillaApp.app
```

The finished bundle is written to `dist/CamillaApp.app`. You can also open `Package.swift` directly in Xcode for development.
...
- CamillaApp does not detect or disable unrelated system-EQ applications.
...
CamillaApp downloads CamillaDSP from its official upstream location during setup. The System Audio Bridge driver is embedded in the application and built from the source in this repository.
...
Issues and pull requests are welcome. When reporting an audio-routing problem, include your macOS version, Mac architecture, physical output device, selected sample rate, and the relevant CamillaApp log without private information.

## License

Copyright © 2026 CamillaApp contributors.

CamillaApp is free software licensed under the [GNU General Public License v3.0 only](LICENSE). It is provided without warranty; see the license for complete terms.
